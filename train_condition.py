...
def iou_metric(y_pred_batch, y_true_batch):
    ...
    for y_pred, y_true in zip(y_pred_batch, y_true_batch):
        ...
        iou += (intersection + 1e-7) / (union - intersection + 1e-7) / y_pred_batch.shape[0]
    return iou
...
def remove_overlap(seg_out, warped_cm):
    ...
    return warped_cm - (torch.cat([seg_out[:, 1:3, :, :], seg_out[:, 5:, :, :]], dim=1)).sum(dim=1, keepdim=True) * warped_cm
