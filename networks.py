...
norm_layer, use_sigmoid, getIntermFeat, Ddropout,
                                       spectral=spectral)
            if getIntermFeat:
...
padding=padw), nn.LeakyReLU(0.2, True)]]
        ...
        for n in range(1, n_layers):
            ...
            if Ddropout:
                sequence += [[
                    self.spectral_norm(nn.Conv2d(nf_prev, nf, 
...
padding=padw)]]
        ...
        if getIntermFeat:
            for n in range(len(sequence)):
                setattr(self, 'model' + str(n), 
nn.Sequential(*sequence[n]))
        else:
            sequence_stream = []
            for n in range(len(sequence)):
...
def define_D(input_nc, ndf=64, n_layers_D=3, norm='instance', 
...
use_sigmoid, num_D, getIntermFeat, Ddownx2,
